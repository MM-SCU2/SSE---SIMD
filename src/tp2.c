...
void correr_filtro_imagen(configuracion_t *config, aplicador_fn_t aplicador) {
    ...
    for (int i = 0; i < config->cant_iteraciones; i++) {
           unsigned long long int starti, endi, cant_ciclos;
           MEDIR_TIEMPO_START(starti);
            aplicador(config);
            MEDIR_TIEMPO_STOP(endi);

            cant_ciclos = endi-starti;
    fprintf(fp, "%llu\n", cant_ciclos);
    }
